...
def compute_cagr(df_group, years):
    ...
    # find closest date
    df_start = df_group[df_group['date'] >= start_date]
    if len(df_start) == 0:
        return np.nan
    nav_start = df_start['nav'].iloc[0]
    nav_end = df_group['nav'].iloc[-1]
    actual_years = (end_date - df_start['date'].iloc[0]).days / 365.25
    if actual_years < 0.5: # not enough data
        return np.nan
...
print("Notebook Performance_Analytics.ipynb generated.")
